...
public class GetNearbyDishesQueryHandler
    : IRequestHandler<GetNearbyDishesQuery, List<DishSummaryDto>>
{
    ...
    public async Task<List<DishSummaryDto>> Handle(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            dishes = dishes.Where(d => d.CategoryId == request.CategoryId.Value).ToList();
        }

        if (request.MaxPrice.HasValue)
        {
            dishes = dishes.Where(d => d.Price <= request.MaxPrice.Value).ToList();
        }
        ...
        var result = dishes
            .Select(d => new DishSummaryDto
            {
                ...
                ChefDisplayName = d.Chef.DisplayName ?? "Chef",
                DistanceKm = 0
            })
